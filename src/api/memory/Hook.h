...
#define ATM_AUTO_REG_HOOK_IMPL(FUNC_PTR, STATIC, CALL, DEF_TYPE, ...)          \
  VA_EXPAND(HOOK_IMPL(                                                         \
      inline memory::HookAutoRegister<DEF_TYPE> DEF_TYPE##AutoRegister,        \
      FUNC_PTR, STATIC, CALL, DEF_TYPE, __VA_ARGS__))

#define ATM_MANUAL_REG_HOOK_IMPL(...) VA_EXPAND(HOOK_IMPL(, __VA_ARGS__))

#define ATM_STATIC_HOOK_IMPL(...)                                              \
  VA_EXPAND(ATM_MANUAL_REG_HOOK_IMPL((*), static, originFunc, __VA_ARGS__))

#define ATM_AUTO_STATIC_HOOK_IMPL(...)                                         \
  VA_EXPAND(ATM_AUTO_REG_HOOK_IMPL((*), static, originFunc, __VA_ARGS__))

#define ATM_INSTANCE_HOOK_IMPL(DEF_TYPE, ...)                                  \
  VA_EXPAND(ATM_MANUAL_REG_HOOK_IMPL((DEF_TYPE::*), , (this->*originFunc),     \
                                     DEF_TYPE, __VA_ARGS__))

#define ATM_AUTO_INSTANCE_HOOK_IMPL(DEF_TYPE, ...)                             \
  VA_EXPAND(ATM_AUTO_REG_HOOK_IMPL((DEF_TYPE::*), , (this->*originFunc),       \
                                   DEF_TYPE, __VA_ARGS__))
 ...
 * DefType::unhook().
 */
#define ATM_TYPED_STATIC_HOOK(DefType, type, priority, identifier, Ret, ...)   \
  VA_EXPAND(ATM_STATIC_HOOK_IMPL(DefType,                                      \
 ...
 * DefType::unhook().
 */
#define ATM_STATIC_HOOK(DefType, priority, identifier, Ret, ...)               \
  VA_EXPAND(                                                                   \
      ATM_STATIC_HOOK_IMPL(DefType, , priority, identifier, Ret, __VA_ARGS__))
 ...
 */
#define ATM_AUTO_TYPED_STATIC_HOOK(DefType, type, priority, identifier, Ret,   \
                                   ...)                                        \
  VA_EXPAND(ATM_AUTO_STATIC_HOOK_IMPL(DefType,                                 \
 ...
 */
#define ATM_AUTO_STATIC_HOOK(DefType, priority, identifier, Ret, ...)          \
  VA_EXPAND(ATM_AUTO_STATIC_HOOK_IMPL(DefType, , priority, identifier, Ret,    \
                                      __VA_ARGS__))
 ...
 * DEF_TYPE::unhook().
 */
#define ATM_TYPED_HOOK(DEF_TYPE, PRIORITY, TYPE, IDENTIFIER, RET_TYPE, ...)    \
  VA_EXPAND(ATM_INSTANCE_HOOK_IMPL(DEF_TYPE,                                   \
                                   : public TYPE, PRIORITY, IDENTIFIER,        \
 ...
 * DEF_TYPE::unhook().
 */
#define ATM_INSTANCE_HOOK(DEF_TYPE, PRIORITY, IDENTIFIER, RET_TYPE, ...)       \
  VA_EXPAND(ATM_INSTANCE_HOOK_IMPL(DEF_TYPE, , PRIORITY, IDENTIFIER, RET_TYPE, \
                                   __VA_ARGS__))
 ...
 */
#define ATM_AUTO_TYPED_INSTANCE_HOOK(DEF_TYPE, PRIORITY, TYPE, IDENTIFIER,     \
                                     RET_TYPE, ...)                            \
  VA_EXPAND(ATM_AUTO_INSTANCE_HOOK_IMPL(DEF_TYPE,                              \
                                        : public TYPE, PRIORITY, IDENTIFIER,   \
                                          RET_TYPE, __VA_ARGS__))

/**
 * @brief Register a hook for an instance function.
 * @details The hook will be automatically registered and unregistered.
 * @see HOOK for usage.
 */
#define ATM_AUTO_INSTANCE_HOOK(DEF_TYPE, PRIORITY, IDENTIFIER, RET_TYPE, ...)  \
  VA_EXPAND(ATM_AUTO_INSTANCE_HOOK_IMPL(DEF_TYPE, , PRIORITY, IDENTIFIER,      \
                                        RET_TYPE, __VA_ARGS__))
